...
/*    @SuppressWarnings("rawtypes")
    ...
    public static List getXmlVO(String xmlDataPath){   //比如地址为   “src/bondInstr.xml"
		List l = null;
        List oList = new ArrayList();; 
        l = (new ParserXml()).parser3Xml(new File(xmlDataPath).getAbsolutePath());
        for (int i = 0; i < l.size(); i++) {
            ...
            Object obj = null;
            try{
            	demo = Class.forName("com.bfm.bean." + key);
            }catch(Exception e){
            	e.printStackTrace();
            }
            try{
